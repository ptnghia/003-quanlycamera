        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Thêm video</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card border-top border-0 border-4 border-primary">
                <div class="card-body p-5 pt-3">
                    <div class="card-title d-flex align-items-center">
                        ...
                        <h5 class="mb-0 font-weight-bold" style="font-size: 16px">Thêm video cần phân tích</h5>
                    </div>
                    <hr>
                    <form class="row g-3" method="POST" action="{{route('video.store')}}" id="upload_video"> 
                        @csrf
                        <div class="col-12">
                            <label for="inputLastName1" class="form-label">Tên video</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror"  value="{{ old('name') }}" name="name" id="name" placeholder="Nhập tên video">
                            @error('name')
                        ...
                        <div class="col-12">
                            ...
                            <textarea class=" form-control @error('note') is-invalid @enderror"  name="note" rows="3" placeholder="Nhập ghi chú...">{{ old('note') }}</textarea>
                            @error('name')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="col-12 d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary px-5"><i class="bx bx-save me-1"></i>Uploade video và phân tích</button>
                        </div>
                    </form>
                </div>
            </div>
