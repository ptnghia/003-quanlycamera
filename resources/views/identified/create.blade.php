        ...
        <div class="col-lg-8 col-12">
            <div class="card border-top border-0 border-4 border-primary">
                <div class="card-body p-5 pt-3">
                    ...
                    <hr>
                    <form class="row g-3" method="POST" action="{{route('identified.store')}}" id="identified"> 
                        @csrf
                        <input type="hidden" name="key_file" value="{{session('key_upload')}}">
                        <div class="col-md-8 col-12">
                            <label class="form-label">Tên đối tượng</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror"  value="{{ old('name') }}"  name="name" placeholder="Nhập tên đối tượng cần giám sá">
                            @error('name')
                        ...
                        <div class="col-md-4 col-12">
                            ...
                            <select class="form-select select2" name="identified_cate_id">
                                ...
                                <option {{old('identified_cate_id ')== $item->id ?'selected':''}} value="{{ $item->id}}">{{ $item->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 col-12">
                            <label for="inputtime" class="form-label">Thời gian</label>
                            <input type="date" class="form-control" id="inputtime" value="{{date('Y-m-d', time())}}">
                        </div>
                        <div class="col-md-6 col-12">
                            <label class="form-label">Phân loại</label>
                            <select class="form-select select2" name="identified_type_id">
                                @foreach ($types as $item)
                                <option {{old('identified_type_id ')== $item->id ?'selected':''}} value="{{ $item->id}}">{{ $item->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12">
